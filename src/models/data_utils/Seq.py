...
class VrptwManager(SeqManager):
	...
	def clone(self):
		...
		res.nodes = []
		for i, node in enumerate(self.nodes):
			res.nodes.append(VrpNode(x=node.x, y=node.y, demand=node.demand, tw_start=node.tw_start, tw_end=node.tw_end, px=node.px, py=node.py, capacity=node.capacity, dis=node.dis, embedding=node.embedding))
		res.num_nodes = self.num_nodes
		res.route = self.route[:]
		res.vehicle_state = self.vehicle_state[:]
		res.tot_dis = self.tot_dis[:]
