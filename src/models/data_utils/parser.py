...
class vrptwParser(object):
	def parse(self, problem, debug=False):
		self.is_debug = debug
		dm = VrpManager(problem['capacity'])
		dm.nodes.append(VrpNode(x=problem['depot'][0], y=problem['depot'][1], demand=0, tw_start=0, tw_end=9999, px=problem['depot'][0], py=problem['depot'][1], capacity=problem['capacity'], dis=0.0))
		for customer in problem['customers']:
			dm.nodes.append(VrpNode(x=customer['position'][0], y=customer['position'][1], demand=customer['demand'], tw_start=customer['time_window'][0], tw_end=customer['time_window'][1], px=customer['position'][0], py=customer['position'][1], capacity=problem['capacity'], dis=0.0))
		dm.num_nodes = len(dm.nodes)
		cur_capacity = problem['capacity']
		pending_nodes = [i for i in range(0, dm.num_nodes)]
		dm.add_route_node(0)
