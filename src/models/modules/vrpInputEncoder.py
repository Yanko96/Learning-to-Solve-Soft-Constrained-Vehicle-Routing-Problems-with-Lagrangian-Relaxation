...
class SeqLSTM(nn.Module):
    ...
    def calc_embedding(self, seq_managers, eval_mode=False):
        ...
        for seq_manager in seq_managers:
            encoder_input.append(data_utils.np_to_tensor(seq_manager.route[:], 'float', self.cuda_flag))
            max_node_cnt = max(max_node_cnt, len(seq_manager.route))
            
        encoder_input = pad_sequence(encoder_input, batch_first=True)
        encoder_input = pack_padded_sequence(encoder_input, [len(seq_manager.route) for seq_manager in seq_managers], batch_first=True, enforce_sorted=False)
        encoder_output, encoder_state = self.encoder(encoder_input)
        encoder_output, lengths = pad_packed_sequence(encoder_output, batch_first=True)

        for seq_manager_idx, seq_manager in enumerate(seq_managers):
            seq_managers[seq_manager_idx].encoder_outputs = encoder_output[seq_manager_idx, :lengths[seq_manager_idx]]

        return seq_managers, encoder_output
